...
function ProjectsPageContent() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const songsRes = await fetch(`/api/songs?projectId=${projectsData.map((p: Project) => p.id).join(',')}`);
  ...
  const handleAddProject = async (projectData: Omit<Project, "id" | "createdAt" | "clientName">) => {
    try {
  ...
  const handleUpdateProject = async (updatedProjectData: Omit<Project, "id" | "createdAt" | "clientName">) => {
  ...
  const handleSongSubmit = async (songData: { name: string; version: string; fileUrl?: string; notes?: string }) => {
    ...
    try {
      const res = await fetch('/api/songs', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ...songData, projectId: selectedProjectForSong.id }),
      });
  ...
  const handleVersionSubmit = async (versionData: { name: string; version: string; fileUrl?: string; notes?: string }) => {
    if (!selectedSongForVersion) return

    try {
      const songToUpdate = songs.find((s) => s.id === selectedSongForVersion.id);
      ...
      const res = await fetch(`/api/songs/${selectedSongForVersion.id}`, {
        ...
        body: JSON.stringify({
          ...
          versions: [...songToUpdate.versions, versionData],
        }),
      });
      if (!res.ok) throw new Error('Failed to add version');
      const updatedSong = await res.json();
      setSongs((prev) => prev.map((s) => (s.id === selectedSongForVersion.id ? updatedSong : s)));
